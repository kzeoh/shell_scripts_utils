#!/bin/bash
filepath=/home/kwonje/lock/4containers/
size=$1
time=100sec

for p in 1 
do
	free -m
	sync
	echo 3 > /proc/sys/vm/drop_caches
	free -m
#	echo 0 > /proc/lock_stat
#	echo 1 > /proc/sys/kernel/lock_stat
	
	for weight in 100 100 100 100 200 200 200 200 300 300 300 300 400 400 400 400 600 600 600 600 700 700 700 700 800 800 800 800 900 900 900 900
	do
		docker run -i --rm --blkio-weight=${weight} -v /volume/workloads/:/mnt --entrypoint="filebench" keinoh/journal:1.5  -f /mnt/fileserver-new.f &
	#	docker run -i --rm --entrypoint="filebench" keinoh/journal:1.4  -f /home/filebench/workloads/fileserver-new-200k.f &
done
	wait
#	echo 0 > /proc/sys/kernel/lock_stat
#	cp /proc/lock_stat ${filepath}${p}_original.txt
#	cat ${filepath}${p}.txt | grep "zone->lock" >> ${filepath}result_${size}_${time}_original.txt
#	rm ${filepath}${p}.txt
done


